...
export class versionminusClient {
  private axios: AxiosInstance;
  private logger: versionminusLogger;

  constructor(private config: versionminusConfig) {
    ...
    if (logRequests) {
      this.axios.interceptors.request.use((req) => {
        try {
          ...
          this.logger.info?.(`[versionminus SDK ${VERSION}] →`, method, url, {
            params: req.params,
            data: req.data,
            timeout: req.timeout,
          });
      ...
      this.axios.interceptors.response.use(
        (res) => {
          try {
            ...
            this.logger.info?.(`[versionminus SDK ${VERSION}] ←`, res.status, method, url, {
        ...
        (error: AxiosError) => {
          try {
            ...
            if (error.response) {
              ...
              this.logger.error?.(`[versionminus SDK ${VERSION}] ✕`, error.response.status, cfg.method?.toUpperCase(), url, {
                data,
                ...(maybeModel ? { model: maybeModel } : {}),
              });
            } else if (error.request) {
              this.logger.error?.(`[versionminus SDK ${VERSION}] ✕ NO_RESPONSE`, cfg.method?.toUpperCase(), url, {
                message: error.message,
              });
            } else {
              this.logger.error?.(`[versionminus SDK ${VERSION}] ✕ REQUEST_SETUP`, { message: error.message });
            }
          } catch { /* ignore logging errors */ }
  ...
  async createNote(input: NoteInput, opts?: { embed?: boolean; model?: string; refresh?: boolean }): Promise<Note> {
    ...
    if (shouldEmbed) {
      try {
        ...
        this.logger.warn?.('[versionminus SDK] note embedding failed (create)', e?.message || e);
      }
    }
    return note;
  }
  ...
  async updateNote(id: string, input: Partial<NoteInput>, opts?: { embed?: boolean; model?: string; refresh?: boolean; force?: boolean }): Promise<Note> {
    ...
    if (shouldEmbed && (contentChanged || opts?.force)) {
      try {
        ...
        this.logger.warn?.('[versionminus SDK] note embedding failed (update)', e?.message || e);
      }
    }
    return note;
  }
...
export function createversionminusClient(config: versionminusConfig) {
  return new versionminusClient(config);
}
