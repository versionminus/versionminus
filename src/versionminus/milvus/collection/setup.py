...
`src/versionminus/milvus/collection/config/` and ensures that each collection and
...
use the unified `versionminus.core.config.Settings` for host/port resolution. The
rest of the codebase (e.g. runtime embeddings scripts) already uses
`Settings`.

Usage (container startup or ad-hoc):
        from versionminus.milvus.collection.setup import sync_collections
        sync_collections()
