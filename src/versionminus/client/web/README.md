...
The SDK supplies a default API base URL: `http://versionminus-api:8000`. Override later by passing `baseUrl` to `useversionminus({ baseUrl: 'https://api.example.com' })` if needed. No `.env` required for local usage.

## Local Development

```bash
# Build the SDK (once or when you change it)
cd src/versionminus/sdk/ts
npm install
npm run build
...
Visit <http://localhost:5173> and ensure the API is reachable at `http://versionminus-api:8000` (started via compose).
...
```bash
cd src/versionminus/client/web
