...
from licodex.core.milvus.milvus import get_milvus  # centralized resilient connection
import json
...
def ensure_notes_collection():
    ...
    Preference order for definition:
      1. JSON config file at licodex/milvus/collection/config/notes.json
      2. Fallback hard-coded schema + IVF_FLAT index
    """
    name = "notes"
    config_path = Path("/opt/licodex/milvus/collection/config/notes.json")  # inside container
