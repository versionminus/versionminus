...
async def retrieve_relevant_notes(session: AsyncSession, *, user_query: str, limit: int = 3) -> list[tuple[uuid.UUID, str]]:
    ...
    async def _fallback_substring() -> list[tuple[uuid.UUID, str]]:
    ...
    try:  # pragma: no cover - relies on external Milvus + model service
        ...
        for nid, (_dist, _edata) in ordered:
            ...
            results.append((nid, snippet if snippet else "(empty note)"))
